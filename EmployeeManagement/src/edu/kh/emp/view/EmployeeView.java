...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				sc.nextLine(); // 입력 버퍼에 남ㅇ있는 잘못 입력된 문자열 제거해서
							   // 무한 반복 방지
			}
		} while (input != 0);
	}
	...
	public void insertEmployee() {
		...
		Employee emp = new Employee(empId, empName, empNo, email, phone, email, phone, salary,
								jobCode, deptCode, salLevel, bonus, managerId);
		
		int result = dao.insertEmployee(emp);
		// INSER, UPDATE, DELETE 같은 DML 구문은
	...
	public void deleteEmployee() {
		System.out.println("<사원이 일치하는 사원 정보 삭제>");
		int empId = inputEmpId();
	...
	public void selectDeptEmp() {
		System.out.println("부서가 일치하는 사원 정보 조회");
		System.out.print("부서명 입력 : ");
		String deptTitle = sc.next();
		
		printAll(dao.selectDeptEmp(deptTitle));
	}
	
	/** 입력 받은 급여 이상을 받는 사원 조회
	 * 
	 */
	public void selectSalaryEmp() {
		System.out.println("<입력 받은 급여 이상을 받는 사원 조회>");
	...
	public void selectDeptTotalSalary() {
		...
		Map<String, Integer> salaryHash = dao.selectDeptTotalSalary();
		if(salaryHash.isEmpty()) {
			System.out.println("조회 된 결과가 없습니다.");
			return;
		}
		System.out.println("|  부서명  |    급여 합    |");
	...
	public void selectJobAvgSalary() {
		...
		Map<String, Double> avgSalHash = dao.selectJobAvgSalary();
		
		if(avgSalHash.isEmpty()) {
			System.out.println("조회된 결과가 없습니다.");
			return;
		}
		System.out.println("|  직급 명  |    급여 합    |");
		for(String key : avgSalHash.keySet()) {
			System.out.printf("%7s|%10.1f\n", key, avgSalHash.get(key));
		}
		System.out.println();
	}
	 ...
	 * @return
	 */
	public int inputEmpId() {		// 해당 테이블의 컬럼을 식별할 수 있는 메인 키는 사용할 일이 많으므로 따로 메서드 생성
