...
public class JDBCExample {
	...
	public static void main(String[] args) {
		...
		// 1단계 : JDBC 객체 참조 변수 선언 (java.sql의 인터페이스)
		//		 -> 
		Connection conn = null;
		// DB 연결 정보를 담은 객체 
		// -> DBMS 타입, 이름, IP, port, 계정명, 비밀번호가 담김
		...
		try {
			// 2 단계 : 참조 변수에 알맞은 객체 대입
			
			// - 1. DB 연결에 필요한 Oracle JDBC Driver 메모리에 로드하기(객체로 만들어 두기)
			Class.forName("oracle.jdbc.driver.OracleDriver");
			// -> 인자로 작성된 클래스의 객체를 반환 
			// -> 메모리에 객체가 생성되어지도 JDBC 필요 시 알아서 참조해서 사용
			...
			// oracle.jdbc.driver.T4Conection@~~~~~~ 나오면 커넥션 생성 성공
			...
			// 3 단계 : SQL을 수행해서 반환 받은 결과(ResultSet)를
			//         한 행씩 접근해서 컬럼 값 얻어오기
			
			while(rs.next()) {
				// rs.next() : rs가 차모하는 resultSet 객체
				//          첫 번째 컬럼부터 순서대로 한 행식 이동하며
